...
for team_name, team_id in teams.items():
    ...
    for inc in incidents:
        ...
        # Determine NACK_at in IST
        nack_dt = utc_to_ist(inc.get("NACK_at"))
        nack_date_str = nack_dt.strftime("%Y-%m-%d") if nack_dt else ""

        # Assignees
        assignees = inc.get("assignee", [])
        assignee_emails = ", ".join(a.get("email", "") for a in assignees if isinstance(a, dict) and a.get("email"))
        ...
        for note in grouped.get("notes", []):
            ...
            if not note_dt:
                continue
            # Only include note if same date as NACK_at
        ...
        for comm in grouped.get("comments", []):
            if isinstance(comm, dict):
                comm_dt = utc_to_ist(comm.get("createdAt"))
                user_disp = resolve_user(comm.get("user"), team_id)
                comm_text = comm.get("content", "").replace("\n", " ")
                all_comments_list.append(f"{ist_str(comm_dt)} | {user_disp}: {comm_text}")
            else:
                all_comments_list.append(str(comm))
